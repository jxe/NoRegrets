...
function checkHistoryLength() //checks history length and delete old data if needed
{
    var options = checkOptions();
    var length = parseInt(options.history_size);
    
    if (length > 0) //if history length restrictions set
    {
        //creating dates string array
        var dateStrings = getDateStrings();

        if (dateStrings.length > length)
        {
            var visits = getVisits();
            //delete old history
            for (i=0; i < dateStrings.length - length; i++)
            {
                delete visits[dateStrings[i]];
            }
...
function checkOptions()
{
    ...
    if (localStorage["options"])
    {
        ...
        // users migrating from 0.6 beta defaults popup action to today's graph
        if (options.default_popup_action == undefined)
        {
            options.default_popup_action = "today";
            needUpdate = true;
        }
        ...
        // options update in 1.0.1 and higher
        var i, stringToArray = ['hidden_domains', 'more_than_2_levels_domains'];
        for (i in stringToArray)
        {
            o = stringToArray[i];
            if (Object.prototype.toString.call(options[o]) !== '[object Array]')
            {
                var newArray = optionsDefault[o];

                var current = options[o].split(","), c;
                for (c in current)
                {
                    newArray.push($.trim(current[c]));
                }
                options[o] = getUnique(newArray);
                needUpdate = true;
            }
        }

        // update categories and dashboard items for users migrating from 1.0.1 (note: version will be updated in wips.js when document is loaded)
        if (localStorage['version'] == '1.0.1')
        {
            options = checkDomainCategories(options);

            while (options.dashboard_items.length < optionsDefault.dashboard_items.length)
            {
                options.dashboard_items.push(optionsDefault.dashboard_items[options.dashboard_items.length]);
            }

            needUpdate = true;
        }

        // add multilevel domains for users migrating from 1.0.3
        if (localStorage['version'] == '1.0.3')
        {
            o = 'more_than_2_levels_domains';
            options[o].push('com.au');
            options[o] = getUnique(options[o]);
            needUpdate = true;
        }

        function isHigherThan(version){
            var versionNumbers = localStorage['version'].split(".");
            var localVersionNums = version.split(".");
            if(versionNumbers[0] > localVersionNums[0]) {
                return true;
            }
            if(versionNumbers[0] == localVersionNums[0] && versionNumbers[1] > localVersionNums[1]){
                return true;
            }
            if(versionNumbers[0] == localVersionNums[0] && versionNumbers[1] == localVersionNums[1] &&
                versionNumbers[2] > localVersionNums[2]){
                return true;
            }
            return false;
        }

        //  convert stored date format for those migrating from 1.0.5
        
        if(!wips.getPref('visits') && !isHigherThan('1.0.4'))
        {
            var visits = {};
            var oldDatePattern = /^[0-9 ]{2}\.[0-9]{2}\.[0-9]{4}$/;
            var date, dateObject, key, item;
            
            for (var key in localStorage)
            {
                if (oldDatePattern.test(key))
                {
                    item = storage_get(key);
                    
                    if (isEmpty(item) == false)
                    {
                        dateObject = getDateObject(key, 'dd.mm.yy');
                        date = getFormatedDate(dateObject);                    
                        visits[date] = item;
                    }
                }
